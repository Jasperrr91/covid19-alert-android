...
internal class RegisterUseCaseImp(
    ...
    override suspend fun register() {
        val response = registerApi.register(RegisterRequest(UUID.randomUUID().toString(), authorizationProvider.provideFcmToken()))
        beaconSetup.setMajorMinor(response.major.toInt(), response.minor.toInt())
        authorizationSaver.saveAuthData(AuthorizationData(response.token(), null))
    }
}
